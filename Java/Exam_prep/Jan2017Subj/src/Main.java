...
public class Main {
    ...
    public static void main(String[] args) throws IOException, NoSuchAlgorithmException, InvalidAlgorithmParameterException, NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException, InvalidKeyException, CertificateException, KeyStoreException, UnrecoverableKeyException, SignatureException {
        File encMessage = new File("EncryptedMessage.cipher");
        if(!encMessage.exists()) {
            throw new RuntimeException("File doesn t exist");
        } else {
            byte[] fileBytes = Files.readAllBytes(encMessage.toPath());
            byte[] hashedFile = getHash(fileBytes, "SHA-1");
            System.out.println("Hash: " + getHexString(hashedFile));
        }

        byte AES_KEY[] = { 0x2A, 0x4D, 0x61, 0x73, 0x74, 0x65, 0x72, 0x20, 0x49, 0x53, 0x4D,
                0x20, 0x32, 0x30, 0x31, 0x37 };
        decrypt("EncryptedMessage.cipher", "OriginalMessage.txt", AES_KEY, "AES");

        KeyStore ks = getKeyStore(
                "ismkeystore.ks", "Secret1234ABCDEF", "pkcs12");
        list(ks);

        PublicKey pubIsm1 = getPublicKey("ismexam1", ks);
        PrivateKey privIsm1 = getPrivateKey("ismexam1", "Secret1234ABCDEF", ks);

        PublicKey pubIsm1FromCert = getCertificateKey("ISMCertificateX509.cer");

        byte[] signature = signFile("OriginalMessage.txt", privIsm1);

        File sgnFile = new File("FileSignature.ds");
        FileOutputStream fos = new FileOutputStream(sgnFile);
        ...
        if(hasValidSignature(
                "OriginalMessage.txt", pubIsm1FromCert, signature))
        {
